...
function NewRecordForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            await fetchWithAuth('/api/medical-records', { method: 'POST', body: JSON.stringify(formData) });
    ...
    return (
        <div className="max-w-2xl mx-auto animate-fadeIn">
            <div className="mb-6">
                ...
                <h1 className="page-header">New Medical Record</h1>
                <p className="page-subtitle">Create a new medical record for a patient</p>
            </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" value={formData.diagnosis} onChange={(e) => setFormData({ ...formData, diagnosis: e.target.value })}
                            className="input-field" placeholder="Enter diagnosis..." required />
                    </div>
                    <div>
                        <label className="form-label">Prescription</label>
                        <textarea value={formData.prescription} onChange={(e) => setFormData({ ...formData, prescription: e.target.value })}
                            className="input-field min-h-[100px] resize-none" placeholder="Enter prescription details..." />
                    </div>
                    <div>
                        <label className="form-label">Notes</label>
                        <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
                            className="input-field min-h-[100px] resize-none" placeholder="Additional notes..." />
                    </div>
